...
feature -- Access
	...
	en_kucuk_asal: INTEGER = 2

	en_kucuk_tek_asal: INTEGER = 3

	higher_prime (n: INTEGER): INTEGER
			-- Lowest prime greater than or equal to `n'
		do
			if n <= en_kucuk_asal then
				Result := en_kucuk_asal
			else
					check
						n > en_kucuk_asal
					end
				from
					if n \\ en_kucuk_asal = 0 then
	...
	lower_prime (n: INTEGER): INTEGER
		...
		require
			argument_big_enough: n >= en_kucuk_asal
		do
			if n = en_kucuk_asal then
				Result := en_kucuk_asal
			else
	...
	is_prime (n: INTEGER): BOOLEAN
		...
		do
			if n <= 1 then
				Result := False
			elseif n = en_kucuk_asal then
				Result := True
			elseif n \\ en_kucuk_asal /= 0 then
				from
					divisor := en_kucuk_tek_asal
				until
					(n \\ divisor = 0) or else (divisor * divisor >= n)
				loop
					divisor := divisor + en_kucuk_asal
				end
